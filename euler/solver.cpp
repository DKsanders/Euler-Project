#include <vector>
#include <string>

#include "solver.h"
#include "math_utils.h"

void Solver::problem1()
{
    unsigned int sum = 0;
    for (int i = 0; i < 1000; ++i)
    {
        if (i % 3 == 0 || i % 5 == 0)
        {
            sum += i;
        }
    }
    std::cout << sum << std::endl;
}

void Solver::problem2()
{
    // 2 of the previous Fibonacci terms
    unsigned int prev1 = 0;
    unsigned int prev2 = 1;

    unsigned int sum = 0;
    while (prev2 < 4000000)
    {
        unsigned int current = prev1 + prev2;
        if (current % 2 == 0)
        {
            sum += current;
        }
        prev1 = prev2;
        prev2 = current;
    }
    std::cout << sum << std::endl;
}

void Solver::problem3()
{
    std::vector<unsigned int> factors;
    std::vector<unsigned int> powers;
    prime_factor_decomposition(600851475143, factors, powers);
    unsigned int largest_prime_factor = factors[factors.size() - 1];

    std::cout << largest_prime_factor << std::endl;
}

void Solver::problem4()
{
    int n = 3;
    unsigned int lower_bound = 0;
    unsigned int uppwer_bound = pow(10, n);
    unsigned int max_palindrome = 0;
    for (int i = uppwer_bound; i > lower_bound; --i)
    {
        for (int j = i; j > lower_bound; --j)
        {
            unsigned int prod = i * j;
            if (is_palindrome(prod))
            {
                if (prod > max_palindrome)
                {
                    max_palindrome = prod;
                    lower_bound = j;
                }
            }
        }
    }

    std::cout << max_palindrome << std::endl;
}

void Solver::problem5()
{
    unsigned int n = 20;
    std::vector<unsigned int> factors;
    for (unsigned int i = 0; i <= n; ++i)
    {
        factors.push_back(i);
    }
    unsigned long long lcm = least_common_multiple(factors);

    std::cout << lcm << std::endl;
}

void Solver::problem6()
{
    unsigned long long n = 100;
    unsigned long long square_of_sum = pow((n * n + n) / 2, 2);
    unsigned long long sum_of_square = 0;
    for (int i = 0; i <= n; ++i)
    {
        sum_of_square += (i * i);
    }
    unsigned long long sum_sq_diff = square_of_sum - sum_of_square;
    std::cout << sum_sq_diff << std::endl;
}

void Solver::problem7()
{
    unsigned int n = 10001;
    unsigned int count = 0;
    unsigned long long candidate = 0;
    while (count < n)
    {
        ++candidate;
        if (is_prime(candidate))
        {
            ++count;
        }
    }
    std::cout << candidate << std::endl;
}

void Solver::problem8()
{
    int consec = 13;
    std::string n = "73167176531330624919225119674426574742355349194934"
        "96983520312774506326239578318016984801869478851843"
        "85861560789112949495459501737958331952853208805511"
        "12540698747158523863050715693290963295227443043557"
        "66896648950445244523161731856403098711121722383113"
        "62229893423380308135336276614282806444486645238749"
        "30358907296290491560440772390713810515859307960866"
        "70172427121883998797908792274921901699720888093776"
        "65727333001053367881220235421809751254540594752243"
        "52584907711670556013604839586446706324415722155397"
        "53697817977846174064955149290862569321978468622482"
        "83972241375657056057490261407972968652414535100474"
        "82166370484403199890008895243450658541227588666881"
        "16427171479924442928230863465674813919123162824586"
        "17866458359124566529476545682848912883142607690042"
        "24219022671055626321111109370544217506941658960408"
        "07198403850962455444362981230987879927244284909188"
        "84580156166097919133875499200524063689912560717606"
        "05886116467109405077541002256983155200055935729725"
        "71636269561882670428252483600823257530420752963450";

    std::vector<bool> is_zero(n.length());
    for (int i = 0; i < n.size(); ++i)
    {
        if (n[i] == '0')
        {
            for (int j = 0; j < consec; ++j)
            {
                if (i - j >= 0)
                {
                    is_zero[i - j] = true;
                }
            }
        }
    }

    unsigned long long current_max = 0;
    unsigned long long current_prod = 0;
    for (int i = 0; i < n.size() - consec + 1; ++i)
    {
        if (!is_zero[i])
        {
            if (current_prod == 0)
            {
                current_prod = 1;
                for (int j = 0; j < consec; ++j)
                {
                    current_prod *= (n[i + j] - '0');
                }
            }
            else
            {
                current_prod = current_prod / (n[i - 1] - '0') * (n[i + consec - 1] - '0');
            }

            if (current_prod > current_max)
            {
                current_max = current_prod;
            }
        }
        else
        {
            current_prod = 0;
        }
    }
    std::cout << current_max << std::endl;
}

void Solver::problem9()
{
    for (unsigned long long a = 1; a < 334; ++a)
    {
        for (unsigned long long b = a + 1; a + b < 667; ++b)
        {
            unsigned long long c = 1000 - a - b;
            if (c > b)
            {
                unsigned long long a_sq = a * a;
                unsigned long long b_sq = b * b;
                unsigned long long c_sq = c * c;
                if (a_sq + b_sq == c_sq)
                {
                    std::cout << a * b * c << std::endl;
                    break;
                }
            }
        }
    }
}

void Solver::problem10()
{
    unsigned long long sum = 2;
    for (unsigned long long i = 3; i < 2000000; i = i + 2)
    {
        if (is_prime(i))
        {
            sum += i;
        }
    }
    std::cout << sum << std::endl;
}

void Solver::problem11()
{
    std::vector<std::vector<int>> grid = {
        {8, 02, 22, 97, 38, 15, 00, 40, 00, 75, 04, 05, 07, 78, 52, 12, 50, 77, 91, 8},
        {49, 49, 99, 40, 17, 81, 18, 57, 60, 87, 17, 40, 98, 43, 69, 48, 04, 56, 62, 00},
        {81, 49, 31, 73, 55, 79, 14, 29, 93, 71, 40, 67, 53, 88, 30, 03, 49, 13, 36, 65},
        {52, 70, 95, 23, 04, 60, 11, 42, 69, 24, 68, 56, 01, 32, 56, 71, 37, 02, 36, 91},
        {22, 31, 16, 71, 51, 67, 63, 89, 41, 92, 36, 54, 22, 40, 40, 28, 66, 33, 13, 80},
        {24, 47, 32, 60, 99, 03, 45, 02, 44, 75, 33, 53, 78, 36, 84, 20, 35, 17, 12, 50},
        {32, 98, 81, 28, 64, 23, 67, 10, 26, 38, 40, 67, 59, 54, 70, 66, 18, 38, 64, 70},
        {67, 26, 20, 68, 02, 62, 12, 20, 95, 63, 94, 39, 63, 8, 40, 91, 66, 49, 94, 21},
        {24, 55, 58, 05, 66, 73, 99, 26, 97, 17, 78, 78, 96, 83, 14, 88, 34, 89, 63, 72},
        {21, 36, 23, 9, 75, 00, 76, 44, 20, 45, 35, 14, 00, 61, 33, 97, 34, 31, 33, 95},
        {78, 17, 53, 28, 22, 75, 31, 67, 15, 94, 03, 80, 04, 62, 16, 14, 9, 53, 56, 92},
        {16, 39, 05, 42, 96, 35, 31, 47, 55, 58, 88, 24, 00, 17, 54, 24, 36, 29, 85, 57},
        {86, 56, 00, 48, 35, 71, 89, 07, 05, 44, 44, 37, 44, 60, 21, 58, 51, 54, 17, 58},
        {19, 80, 81, 68, 05, 94, 47, 69, 28, 73, 92, 13, 86, 52, 17, 77, 04, 89, 55, 40},
        {04, 52, 8, 83, 97, 35, 99, 16, 07, 97, 57, 32, 16, 26, 26, 79, 33, 27, 98, 66},
        {88, 36, 68, 87, 57, 62, 20, 72, 03, 46, 33, 67, 46, 55, 12, 32, 63, 93, 53, 69},
        {04, 42, 16, 73, 38, 25, 39, 11, 24, 94, 72, 18, 8, 46, 29, 32, 40, 62, 76, 36},
        {20, 69, 36, 41, 72, 30, 23, 88, 34, 62, 99, 69, 82, 67, 59, 85, 74, 04, 36, 16},
        {20, 73, 35, 29, 78, 31, 90, 01, 74, 31, 49, 71, 48, 86, 81, 16, 23, 57, 05, 54},
        {01, 70, 54, 71, 83, 51, 54, 69, 16, 92, 33, 48, 61, 43, 52, 01, 89, 19, 67, 48},
    };

    unsigned long long current_max = 0;

    // Rows
    for (int i = 0; i < grid.size(); ++i)
    {
        for (int j = 0; j < grid[0].size() - 3; ++j)
        {
            unsigned long long current_prod = 1;
            for (int k = 0; k < 4; ++k)
            {
                current_prod *= grid[i][j + k];
            }
            if (current_prod > current_max)
            {
                current_max = current_prod;
            }
        }
    }
    // Columns
    for (int i = 0; i < grid[0].size(); ++i)
    {
        for (int j = 0; j < grid.size() - 3; ++j)
        {
            unsigned long long current_prod = 1;
            for (int k = 0; k < 4; ++k)
            {
                current_prod *= grid[j + k][i];
            }
            if (current_prod > current_max)
            {
                current_max = current_prod;
            }
        }
    }
    // Left-to-right diagonal
    for (int i = 0; i < grid.size() - 3; ++i)
    {
        for (int j = 0; j < grid[0].size() - 3; ++j)
        {
            unsigned long long current_prod = 1;
            for (int k = 0; k < 4; ++k)
            {
                current_prod *= grid[i + k][j + k];
            }
            if (current_prod > current_max)
            {
                current_max = current_prod;
            }
        }
    }
    // Left-to-right diagonal
    for (int i = 0; i < grid.size() - 3; ++i)
    {
        for (int j = 3; j < grid[0].size(); ++j)
        {
            unsigned long long current_prod = 1;
            for (int k = 0; k < 4; ++k)
            {
                current_prod *= grid[i + k][j - k];
            }
            if (current_prod > current_max)
            {
                current_max = current_prod;
            }
        }
    }
    std::cout << current_max << std::endl;
}

void Solver::problem12()
{
    unsigned long long triangle_num = 0;
    for (int i = 1; true; ++i)
    {
        triangle_num += i;
        std::vector<unsigned long long> const& factors = get_factors(triangle_num);
        std::cout << triangle_num << std::endl;
        if (factors.size() > 500)
        {
            break;
        }
    }
    std::cout << triangle_num << std::endl;
}